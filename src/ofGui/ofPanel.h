...
public:
	...
	virtual void touchDown(ofTouchEventArgs &touch){
		setValue(touch.x, touch.y, true);
		if( bGuiActive ){
			ofTouchEventArgs a = touch;
			a.x -= b.x;
			a.y -= b.y;
			for(int i = 0; i < collection.size(); i++){
				collection[i]->touchDown(a);
			}
		}
	}
	
	virtual void touchMoved(ofTouchEventArgs &touch){
		setValue(touch.x, touch.y, false);	
		if( bGuiActive ){
			ofTouchEventArgs a = touch;
			a.x -= b.x;
			a.y -= b.y;			
			for(int i = 0; i < collection.size(); i++){
				collection[i]->touchMoved(a);
			}
		}
	}
